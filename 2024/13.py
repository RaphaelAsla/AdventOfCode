...
def main(aocd: AOCD):
    ...
    for entry in data:
        ...
        buttons = [[x1_inc, x2_inc], [y1_inc, y2_inc]]
        prize = entry[2]
        x = int(prize.split("X=")[1].split(",")[0])
        y = int(prize.split("Y=")[1])
        buttons_nprize.append([np.linalg.inv(buttons), np.array([x, y])])

    error = 0.01
    c = 10000000000000
    for res in buttons_nprize:
        p1b1, p1b2 = np.dot(res[0], res[1])
        p2b1, p2b2 = np.dot(res[0], res[1] + c)
